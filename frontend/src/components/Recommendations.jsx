...
export default function Recommendations({ isLoggedIn, setCurrentPage, setSelectedBook }) {
  ...
  useEffect(() => {
    ...
    fetch('http://127.0.0.1:8000/api/recommendations/', { headers })
      .then(res => {
    ...
    <section className="py-12 flex flex-col items-center">
      ...
      <div className="flex items-center justify-center gap-4 mb-12">
        ...
        <h2 className="text-3xl sm:text-4xl font-bold italic font-serif text-theme-secondary transition-colors duration-500">
          {isLoggedIn ? "Спеціально для вас" : "Книжкові рекомендації"}
        </h2>
            ...
            {recommendations.map((book) => (
              ...
              >
                ...
                <p className="leading-relaxed flex-grow opacity-80 text-sm sm:text-base">
                  <span dangerouslySetInnerHTML={{ __html: book.description || "Опис для цієї книги з'явиться згодом. Залишайтеся з нами!" }} />
                </p>
              </div>
            ))}
          </div>
      ...
      {!isLoading && !error && recommendations.length === 0 && (
        <p className="text-theme-secondary opacity-70 italic">Для вас поки немає нових рекомендацій.</p>
      )}
    </section>
  );
}